...
const AddReceived = () => {
    ...
    const addReceived = async (formData: FormData) => {
        const productNumber = formData.get("productNumber");

        const receivedDateUnix = Math.floor(new Date().getTime() / 1000);

        if (walletProvider){
            const provider = new BrowserProvider(walletProvider);
            const signer = await provider.getSigner();
            const signature = await signer?.signMessage(`Receive the specified product`)
            ...
            try {
                ...
                const transaction = await contract.addReceivedDate(
                    productNumber,
                    receivedDateUnix
                );
                ...
                if (receipt.status === 1) {
                    ...
                    router.push("/reload?path=retailer")
                } else {
    ...
    return (
        <form action={addReceived} className="flex flex-col bg-white p-8 rounded shadow-md shadow-green-800 w-full">
            <h2 className="text-2xl font-bold mb-4 text-center">Receive a Product</h2>
            <div className="mb-4">
